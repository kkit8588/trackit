<?php 
    include '../connect.php';
    $id = 1;
    $select = mysqli_query($conn, "SELECT * FROM req_tbl ORDER BY id DESC");
    ...
    <tr>
        ...
        <td><?php echo $row ['yg'];?></td>
        <td><?php echo $row ['email'];?></td>
        <td>
            <ul>
                <?php
                    $select3 = mysqli_query($conn, "SELECT * FROM req_name WHERE req_num = '".$row['req_num']."'");
                    while ($row3 = $select3->fetch_assoc()) {
                ?>
                        <li><?php echo $row3['rn'];?></li>
                <?php } ?>
            </ul>
        </td>
        <td>
            <?php
                $dateTimeManila = new DateTime($row['request_date']);
                $words = $dateTimeManila->format('l, F j, Y');
